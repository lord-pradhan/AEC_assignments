...
public:
	...
	// Everything below written by Roshan Pradhan
	...
	void TreeToVine()
	{
		auto psdHd = MakeHandle(pseudo_root);
		auto rootHd =  RootNode() ;
		
		GetNode(psdHd)->right = GetNode(rootHd);

		if(rootHd.IsNotNull()){
			
			GetNode(rootHd)->up = GetNode(psdHd);
		}

		auto tailHd = psdHd;
		auto restHd = Right(tailHd);

		while(restHd.IsNotNull()){

			if(Left(restHd).IsNull()){
				...
				auto tempHd = Left(restHd);
				GetNode(restHd)->left = GetNode(Right(tempHd));

				if(Right(tempHd).IsNotNull()){
					
					GetNode(Right(tempHd))->up = GetNode(restHd);
				}

				GetNode(tempHd)->right = GetNode(restHd);

				if(restHd.IsNotNull()){

					GetNode(restHd)->up = GetNode(tempHd);
				}

				restHd = tempHd;
				GetNode(tailHd)->right = GetNode(tempHd);

				if(tempHd.IsNotNull()){
					GetNode(tempHd)->up = GetNode(tailHd);
				}
			}
		}
	...
	void Compress( Node* pseudo_root_, int count)
	{

		auto scanner = pseudo_root_;
		auto scannerHd = MakeHandle(scanner);

		for (int i = 0; i < count; i++){

			if(scannerHd.IsNotNull()){

				auto childHd = Right(scannerHd);
				GetNode(scannerHd)->right = GetNode(Right(childHd));
				if(Right(childHd).IsNotNull()){
					GetNode(Right(childHd))->up = GetNode(scannerHd);
				}

				scannerHd = Right(scannerHd);
				GetNode(childHd)->right = GetNode(Left(scannerHd));
				if(Left(scannerHd).IsNotNull()){
					GetNode(Left(scannerHd))->up = GetNode(childHd);
				}

				GetNode(scannerHd)->left = GetNode(childHd);
				if(childHd.IsNotNull()){
					GetNode(childHd)->up = GetNode(scannerHd);
				}
			}
		}
	}
	...
	void VineToTree()
	{
		if(IsVine()){
			...
			auto rootHd =  RootNode() ;
			auto size = nElem;
			auto leaves = size + 1 - pow( 2, (int)(log2( size + 1 )) );
			...
			cout<<"First convert to vine"<<endl;
		}
	}
...
protected:
	...
	void Rebalance(NodeHandle ndHd){
		...
		auto diff_height = GetHeight(Right(ndHd)) - GetHeight(Left(ndHd));

		//left cases
		if ( diff_height<=-2 ){

			// left-left case
			if ( GetHeight( Left(Left(ndHd)) ) > GetHeight( Right(Left(ndHd)) ) ){

				RotateRight(ndHd);
			}

			// left-right case
			else if( GetHeight( Left(Left(ndHd)) ) < GetHeight( Right(Left(ndHd)) ) ){

				RotateLeft( Left(ndHd) );
				RotateRight(ndHd);
			}
		}
		else if( diff_height>=2 ){

			// right-right case
			if ( GetHeight( Right( Right(ndHd) ) ) > GetHeight( Left(Right(ndHd)) ) ){

				RotateLeft(ndHd);
			}
			// right-left case
			else if( GetHeight( Right(Right(ndHd)) ) < GetHeight( Left(Right(ndHd)) ) ){

				RotateRight( Right(ndHd) );
				RotateLeft(ndHd);
			}
		}

		while (GetNode(ndHd)->up!=nullptr){
			ndHd = Up(ndHd);
			Rebalance(ndHd);
		}
