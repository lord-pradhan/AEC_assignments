...
/* virtual */ void FsLazyWindowApplication::Initialize(int argc,char *argv[])
{
	if(2<=argc)// && true==mesh.LoadBinStl(argv[1]))
	{
		...
		airfoil = MakeNACA4DigitAirfoil(strtol(argv[1], nullptr, 10), std::stod(argv[2]));
		
		for(int i = 0; i < airfoil.size(); i++) {

			// std::cout<<"For loop entered "<<i <<"\n";
			double x,y,z;
		    airfoil[i].Get(x,y,z);
		    AddVertex(vtx, x,y,z);//airfoil[i].x , airfoil[i].y , airfoil[i].z);
