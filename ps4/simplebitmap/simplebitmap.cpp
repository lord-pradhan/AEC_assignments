...
bool SimpleBitmap::operator==(const SimpleBitmap &bitmapB) const
{
	...
	if(this->GetTotalNumComponent()!= bitmapB.GetTotalNumComponent())
		return false;
	else if (this->GetWidth() != bitmapB.GetWidth())
		return false;
	else if (this->GetHeight() != bitmapB.GetHeight())
		return false;
	else{

		for (int i = 0; i< bitmapB.GetTotalNumComponent(); i++){

			if( *(this->GetBitmapPointer()+i) != *(bitmapB.GetBitmapPointer()+i) )
				return false;
		}
	}
...
bool SimpleBitmap::operator!=(const SimpleBitmap &bitmapB) const
{
	...
	// if(nx != bitmapB.nx)
	// 	return true;
	// else if (ny != bitmapB.ny)
	// 	return true;
	// else{

	// 	for (i = 0; i< bitmapB.GetTotalNumComponent(); i++){

	// 		if(bmpPtr[i] != bitmapB.bmpPtr[i])
	// 			return true;
	// 	}
	// }
