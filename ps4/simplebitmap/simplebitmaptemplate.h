...
public:
	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &destination,int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
		...
		// cout<<"Pointer allocated"<<endl;

		for (int i = thisX0; i < thisX0 + wid; i++){

			for (int j = thisY0; j< thisY0 + hei; j++){

				if (i >= this->nx || j >= this->ny){

					for(int k = 0; k<NumComponentPerPixel; k++){

						destination.bmpPtr[NumComponentPerPixel*( (j-thisY0)*wid + (i-thisX0) ) +k] = clearColor;
					}					
				}
					...
					for(int k = 0; k<NumComponentPerPixel; k++){
						...
						destination.bmpPtr[NumComponentPerPixel*( (j-thisY0)*wid + (i-thisX0) ) + k] = this->bmpPtr[NumComponentPerPixel*(j*nx+i) + k];
					}
				}
