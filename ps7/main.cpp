...
protected:
	bool needRedraw;
	int state; // 0 for painting, 1 for slices, 2 for laser scanning
	...
	// MeshLattice meshlatS, meshlatL; 
	// int Nx_S, Ny_S, Nz_S, Nx_L, Ny_L, Nz_L;
	...
	bool paintDFS( YsShellExt& mesh, YsShellExt::PolygonHandle& startPoly , 
...
/* virtual */ void FsLazyWindowApplication::Interval(void)
{
	...
	if(evt==FSMOUSEEVENT_LBUTTONDOWN)
	{
		...
		if(state==0 && plHd!=mesh.NullPolygon() ){

			const double angleTolerance = 20.0*YsPi/180.0;
			
			Paint(mesh, plHd , angleTolerance);
			RemakeVertexArray();
		}
	}
	...
	if(FSKEY_S == key && state!=1){
		state = 1;
		vtx.clear();
		col.clear();

		auto outVect= MakeSliceVertexArray(mesh);

		while(!outVect.empty()){

			auto tempZ = outVect.back();
			outVect.pop_back();
			auto tempY = outVect.back();
			outVect.pop_back();
			auto tempX = outVect.back();
			outVect.pop_back();

			AddVertex(vtx, tempX, tempY, tempZ);
			AddColor(col, 0,0,0,0);
		}
	}

	if(FSKEY_L == key && state!=2){
		state = 2;

		// auto t0_L=FsSubSecondTimer();

		vtx.clear();
		col.clear();

		auto outVectLaser = MakeRayIntersectionVertexArray(mesh);

		while(!outVectLaser.empty()){

			auto tempZ = outVectLaser.back();
			outVectLaser.pop_back();
			auto tempY = outVectLaser.back();
			outVectLaser.pop_back();
			auto tempX = outVectLaser.back();
			outVectLaser.pop_back();
		...
		// printf("time for laser is %d milli seconds\n",(int)FsSubSecondTimer()-t0_L);
	}
